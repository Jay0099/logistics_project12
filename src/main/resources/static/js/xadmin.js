$(function () {
    ...
    //左侧菜单效果
    $('.left-nav #nav>li.list>a').click(function () {
        var par = $(this).parent();
        if($(par).hasClass('open')){
            $(par).removeClass('open');
            $(par).find('.nav_right').html('&#xe697;');
            $(par).children('.sub-menu').slideUp();
            // $(this).siblings().children('.sub-m  enu').slideUp();
        }else{
            $(par).addClass('open');
            $(par).find('.nav_right').html('&#xe6a6;');
            $(par).children('.sub-menu').slideDown();
            $(par).siblings().children('.sub-menu').slideUp();
            $(par).siblings().removeClass('open');
        }
    })
//左侧菜单效果
    $('.left-nav #nav li .sub-menu li.list1').click(function () {
        if($(this).hasClass('open')){
            $(this).removeClass('open');
            $(this).find('.nav_left').html('&#xe6a7;');
            $(this).children('.sub-menu1').slideUp();
            // $(this).siblings().children('.sub-menu').slideUp();
        }else{
            $(this).addClass('open');
            $(this).find('.nav_left').html('&#xe6a6;');
            $(this).children('.sub-menu1').slideDown();
